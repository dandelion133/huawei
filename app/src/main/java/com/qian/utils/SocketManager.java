...
public class SocketManager {
	...
	class UdpSend extends Thread {
		...
		public void run() {
			try {
				byte[] data = SocketManager.toByteArray(msg);
				DatagramSocket ds = new DatagramSocket(SocketManager.PORT_SEND);
				DatagramPacket packet = new DatagramPacket(data, data.length,
						InetAddress.getByName(msg.getReceiveUserIp()),//指定接收方
						SocketManager.PORT_RECEIVE);
				packet.setData(data);
				ds.send(packet);
				ds.close();
	...
	class UdpReceive extends Thread {
		...
		public void run() {
			...
			while (true) {
				try {
					DatagramSocket ds = new DatagramSocket(SocketManager.PORT_RECEIVE);
					...
					Msg msg = (Msg) SocketManager.toObject(data2);
					ds.close();
